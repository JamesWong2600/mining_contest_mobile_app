...
export default function Cheating_report_list() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const response = await fetch(`${config.API_BASE_URL}/report_list/`, {
        method: 'POST',
        body: formData,
      });
      ...
      if (response.ok) {
        setreport(data.report || []); // Use empty array as fallback
      }
    } catch (error) {
      console.error('Error fetching dashboard data:', error);
      setreport([]); // Set empty array on error
    } finally {
  ...
  const handleDeleteReport = async (date) => {
        try {
            setIsLoading(true);  // Start loading
            const response = await fetch(`${config.API_BASE_URL}/report_delete/`, {
                method: 'POST',
                headers: {
    ...
    */
         ...
         const handleReportContent = async (date) => {
                await AsyncStorage.setItem('report_date', date);
                navigation.navigate('Cheating_report_content'); 
        };
              ...
              <View style={styles.contentContainer}>
                {report.map((Report) => (
                  <TouchableOpacity 
                    ...
                    onPress={() => handleReportContent(Report.uploaded_at)}
                  >
                <View style={{ flex: 1 }}>
                  <View style={styles.cardHeader}>
                    <Text style={styles.cardTitle}>舉報記錄: {Report.uploaded_at}</Text>
                    <TouchableOpacity
                      style={{ marginLeft: 8, padding: 4, backgroundColor: '#ff5252', borderRadius: 8 }}
                      onPress={() => handleDeleteReport(Report.created_at)}
                    >
